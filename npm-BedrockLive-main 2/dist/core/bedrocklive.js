    ...
    async loadPlugins() {
        ...
        for (const pluginModuleInfo of this._selectedPlugins) {
            try {
                ...
                console.error(chalk.red(`Failed to load or initialize plugin: ${pluginModuleInfo.manifest.name} (from ${pluginModuleInfo.mainFile})`), error.message, error.stack);
            }
        }
    }
    /**
