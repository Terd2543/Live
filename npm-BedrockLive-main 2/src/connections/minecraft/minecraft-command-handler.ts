...
interface AwaitedQueueItem {
    message: CommandRequest;
    timeoutId: NodeJS.Timeout;
}

export default class MinecraftCommandHandler {
    private _pendingCommandRequests: CommandRequest[] = [];
    private _awaitedQueue: { [key: string]: AwaitedQueueItem; } = {};
    private _timeOut: number = 5000;
    private _minecraftWs: WebSocket | undefined;

    public setActiveConnection(mcWs: WebSocket | undefined): void {
    ...
    public createCommandRequest(command: string): CommandRequest {
        return {
            header: {
                version: 1,
                requestId: uuidv4(),
    ...
    public enqueueCommand(commandRequest: CommandRequest): void {
    ...
    private processSendQueue(): void {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!currentCommandRequest) continue;
            try {
    ...
    public handleCommandResponse(message: any): void {
